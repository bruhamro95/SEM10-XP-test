...
/* Stable Pomodoro multi-task layer. It patches only the TimerApp UI and the
   existing timer link/session metadata; the Tracker and Planner structures are
   not touched. */
const pomodoroMultiTask = {
  ...
  transform(code, id) {
    ...
    // Backwards-compatible storage for a group of linked lecture IDs.
    out = out.replace('soundEnabled: true });', 'soundEnabled: true, linkedIds: [] });');

    // Extend the existing link handler with a safe, immutable multi: value.
    ...
    // Preserve all lecture IDs in the session log while keeping lectureId for
    // older statistics/export code that expects a single ID.
