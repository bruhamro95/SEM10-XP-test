...
(function(){
  ...
  function addOptions(select){
    ...
    const names=new Set();
    document.querySelectorAll('.xp-window .section-header .section-title').forEach(el=>{const n=el.textContent.trim();if(n)names.add(n)});
    if(!names.size)return;
    const existing=new Set(Array.from(select.options).map(o=>o.value));
    names.forEach(n=>{if(!existing.has(n)){const o=document.createElement('option');o.value=n;o.textContent=n;select.appendChild(o)}});
