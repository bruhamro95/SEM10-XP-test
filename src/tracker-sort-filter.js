...
(function installTrackerSortFilter() {
  ...
  const css = `
    .sem10-sf-panel {
      position: fixed; z-index: 100000; width: min(350px, calc(100vw - 20px));
      max-height: min(78vh, 620px); overflow: auto; box-sizing: border-box;
      padding: 10px; background: #ece9d8; border: 2px solid #0a246a;
      box-shadow: 3px 3px 0 rgba(0,0,0,.28); font-family: Tahoma,Arial,sans-serif;
      font-size: 12px; color: #111; pointer-events: auto; touch-action: manipulation;
    ...
    .sem10-sf-field select { width:100%; box-sizing:border-box; min-height:34px; padding:5px 7px; border:1px solid #7f9db9; background:#fff; color:#111; font:13px Tahoma,Arial,sans-serif; touch-action:manipulation; }
    ...
    .sem10-sf-actions button { min-height:32px; padding:5px 11px; border:1px solid #777; border-radius:2px; background:linear-gradient(#fff,#d8d8d8); font:13px Tahoma,Arial,sans-serif; cursor:pointer; touch-action:manipulation; }
    @media (max-width:560px) {
      .sem10-sf-panel { width:calc(100vw - 16px); left:8px !important; right:8px; max-height:calc(100vh - 24px); }
  ...
  function rowInfo(row) {
    ...
    const flagCell = cells[cells.length - 1]; const svg = flagCell?.querySelector("svg");
  ...
  function makeField(labelText, select, full) {
    const wrap=document.createElement("div"); wrap.className="sem10-sf-field"+(full?" full":"");
    const label=document.createElement("label"); label.textContent=labelText; wrap.append(label,select); return wrap;
  }
  function buildPanel(scope, button) {
    ...
    option(stage,"all","Any stage"); STAGES.forEach(([k,l])=>option(stage,k,l));
    ...
    function apply(){
      ...
      rows.forEach(row=>{ const info=rowInfo(row); let show=true; if(selectedChapter!=="all"&&info.section!==selectedChapter)show=false; if(selectedExam!=="all"&&info.exam!==selectedExam)show=false; if(selectedStage!=="all"){const idx=STAGES.findIndex(([k])=>k===selectedStage);const checked=idx>=0?!!info.done[idx]:false;if(selectedState==="todo"&&checked)show=false;if(selectedState==="done"&&!checked)show=false;} if(selectedFlag==="flagged"&&!info.flagged)show=false; if(selectedFlag==="unflagged"&&info.flagged)show=false; row.style.display=show?"":"none"; if(show)visibleCount+=1; });
      ...
      if(selectedSort!=="original")scope.querySelectorAll("table.lecture-table tbody").forEach(tbody=>{Array.from(tbody.querySelectorAll(":scope > tr")).filter(r=>r.querySelector("td.lecture-name-cell")).sort(cmp).forEach(row=>tbody.appendChild(row));});
  ...
  function positionPanel(panel,button){const r=button.getBoundingClientRect();const width=panel.offsetWidth;const maxLeft=Math.max(8,window.innerWidth-width-8);panel.style.left=Math.max(8,Math.min(r.left,maxLeft))+"px";const top=r.bottom+4;panel.style.top=Math.max(8,Math.min(top,window.innerHeight-panel.offsetHeight-8))+"px";}
  ...
  function setupToolbar(toolbar){
    ...
    const scope=findScope(toolbar); if(!scope)return; toolbar.dataset.sem10SortFilterReady="1";
  ...
  // Bubble-phase only. The old capture listener was closing the popup before a
  // mobile tap could reach the select/button inside it.
  ...
  window.addEventListener("resize",()=>{if(openPanel&&!openPanel.hidden)positionPanel(openPanel,openButton);});
