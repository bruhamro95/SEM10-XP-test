...
(function installTrackerSortFilter() {
  ...
  const STAGES = [
    ["paper", "ورق"], ["explain", "شرح"], ["study", "مذاكرة"], ["solve", "حل"], ["review", "مراجعة"],
  ];
  const instances = new WeakMap();
  let openPanel = null;

  const css = `
    .sem10-sf-panel { position: fixed; z-index: 100000; width: min(350px, calc(100vw - 20px)); max-height: min(78vh, 620px); overflow: auto; box-sizing: border-box; padding: 10px; background: #ece9d8; border: 2px solid #0a246a; box-shadow: 3px 3px 0 rgba(0,0,0,.28); font-family: Tahoma, Arial, sans-serif; font-size: 12px; color: #111; }
    .sem10-sf-title { display:flex; align-items:center; justify-content:space-between; gap:8px; margin-bottom:8px; font-weight:700; }
    ...
    .sem10-sf-field select { width:100%; box-sizing:border-box; min-height:26px; padding:3px 5px; border:1px solid #7f9db9; background:#fff; color:#111; font:12px Tahoma, Arial, sans-serif; }
    .sem10-sf-summary { margin:9px 0 7px; padding:5px 6px; background:#fff; border:1px solid #aaa; font-weight:700; }
    .sem10-sf-actions { display:flex; gap:6px; justify-content:flex-end; margin-top:8px; }
    .sem10-sf-actions button { min-height:25px; padding:3px 9px; border:1px solid #777; border-radius:2px; background:linear-gradient(#fff,#d8d8d8); font:12px Tahoma, Arial, sans-serif; cursor:pointer; }
    .sem10-sf-actions button:hover { background:linear-gradient(#fff,#c9d8ef); }
    @media (max-width: 560px) { .sem10-sf-panel { width:calc(100vw - 16px); left:8px !important; right:8px; } .sem10-sf-grid { grid-template-columns:1fr; } .sem10-sf-field.full { grid-column:auto; } }
  `;
  ...
  function lectureRows(scope) { return Array.from(scope.querySelectorAll("table.lecture-table tbody tr")).filter((row) => row.querySelector("td.lecture-name-cell")); }
  ...
  function buildPanel(scope, button) {
    ...
    panel.className = "sem10-sf-panel"; panel.hidden = true;
    ...
    option(sort, "original", "Original order"); option(sort, "name-asc", "Lecture A → Z"); option(sort, "name-desc", "Lecture Z → A"); option(sort, "least", "Least complete first"); option(sort, "most", "Most complete first"); option(sort, "flagged", "Flagged first"); option(sort, "mid", "Midterm first"); option(sort, "final", "Final first");
    ...
    function apply() {
      refreshChapterOptions(); const rows = lectureRows(scope); const selectedChapter = chapter.value, selectedExam = exam.value, selectedStage = stage.value, selectedState = stageState.value, selectedFlag = flagged.value, selectedSort = sort.value; let visibleCount = 0;
      rows.forEach((row) => { const info = rowInfo(row); let show = true; if (selectedChapter !== "all" && info.section !== selectedChapter) show = false; if (selectedExam !== "all" && info.exam !== selectedExam) show = false; if (selectedStage !== "all") { const idx = STAGES.findIndex(([key]) => key === selectedStage); const checked = idx >= 0 ? !!info.done[idx] : false; if (selectedState === "todo" && checked) show = false; if (selectedState === "done" && !checked) show = false; } if (selectedFlag === "flagged" && !info.flagged) show = false; if (selectedFlag === "unflagged" && info.flagged) show = false; row.style.display = show ? "" : "none"; if (show) visibleCount += 1; });
      ...
      if(selectedSort!=="original") scope.querySelectorAll("table.lecture-table tbody").forEach((tbody)=>{const rowsInBody=Array.from(tbody.querySelectorAll(":scope > tr")).filter((r)=>r.querySelector("td.lecture-name-cell")); rowsInBody.sort(cmp).forEach((row)=>tbody.appendChild(row));});
    ...
    reset.addEventListener("click",()=>{chapter.value="all";exam.value="all";stage.value="all";stageState.value="all";flagged.value="all";sort.value="original";apply();});
    close.addEventListener("click",()=>closePanel()); panel.addEventListener("click",(e)=>e.stopPropagation());
    return { panel, apply, refreshChapterOptions };
  }
  function positionPanel(panel,button){const r=button.getBoundingClientRect(); panel.style.left=Math.max(8,Math.min(r.left,window.innerWidth-panel.offsetWidth-8))+"px"; panel.style.top=Math.min(window.innerHeight-panel.offsetHeight-8,r.bottom+4)+"px";}
  function closePanel(){if(!openPanel)return;openPanel.hidden=true;openPanel=null;}
  function setupToolbar(toolbar){
    if(!toolbar||toolbar.dataset.sem10SortFilterReady==="1")return; const scope=findScope(toolbar); if(!scope)return; toolbar.dataset.sem10SortFilterReady="1";
    ...
    button.addEventListener("click",(e)=>{e.stopPropagation();if(openPanel&&openPanel!==built.panel)closePanel();const opening=built.panel.hidden;built.panel.hidden=!opening;openPanel=opening?built.panel:null;if(opening){built.refreshChapterOptions();built.apply();requestAnimationFrame(()=>positionPanel(built.panel,button));}});
    built.apply();
  }
  function scan(){document.querySelectorAll(".toolbar").forEach(setupToolbar);document.querySelectorAll("table.lecture-table").forEach((table)=>{let node=table.parentElement;for(let i=0;i<8&&node;i+=1,node=node.parentElement){const instance=instances.get(node);if(instance){instance.apply();break;}}});}
  document.addEventListener("click",(e)=>{if(!openPanel)return;const target=e.target;if(openPanel.contains(target)||openPanel.button?.contains(target))return;closePanel();},true);
